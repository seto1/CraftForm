...
<?php echo $this->BcForm->create('CraftForm.CraftFormRule') ?>
	<table cellpadding="0" cellspacing="0" class="form-table">
		...
		<tr>
			...
			<td class="col-input">
				<?php echo $this->BcForm->input('name', ['size' => 25]) ?> <small>※半角英数で入力してください</small>
	...
	<table cellpadding="0" cellspacing="0" class="form-table craft-form-rule-table">
		...
		<tr class="mailfield-rules-validateEqualTo">
			<th>一致確認</th>
			<td>
				<?php echo $this->BcForm->input('equalTo', ['size' => 20]) ?><?php echo $this->BcForm->error('equalTo') ?><br>
		...
		<tr class="mailfield-rules-validateLength">
			<th>文字数</th>
			<td>
				<?php echo $this->BcForm->input('minLength', ['size' => 1]) ?> 文字<?php echo $this->BcForm->error('minLength') ?> ～ <?php echo $this->BcForm->input('maxLength', ['size' => 1]) ?> 文字<?php echo $this->BcForm->error('maxLength') ?><br>
			</td>
		</tr>
		<tr class="mailfield-rules-validateCharacterType">
			<th>文字種</th>
			<td>
				<?php echo $this->BcForm->input('characterType', ['type' => 'select', 'multiple' => 'checkbox',
					'options' => [
						'hiragana' => 'ひらがな',
						'katakana' => 'カタカナ',
						'halfKatakana' => '半角カタカナ',
		...
		<tr class="mailfield-rules-validateRange">
			<th>数値</th>
			<td>
				<?php echo $this->BcForm->input('min', ['size' => 1]) ?><?php echo $this->BcForm->error('min') ?> ～ <?php echo $this->BcForm->input('max', ['size' => 1]) ?><?php echo $this->BcForm->error('max') ?><br>
		...
		<tr class="mailfield-rules-validateDate">
			<th>日付</th>
			<td>
				<?php echo $this->BcForm->input('since', ['size' => 7]) ?><?php echo $this->BcForm->error('since') ?> ～ <?php echo $this->BcForm->input('until', ['size' => 7]) ?><?php echo $this->BcForm->error('until') ?><br>
		...
		<tr class="mailfield-rules-validateRegex">
			<th>正規表現</th>
			<td>
				/ <?php echo $this->BcForm->input('regex', ['size' => 50]) ?> /us <?php echo $this->BcForm->error('regex') ?><br>
			</td>
		</tr>
		<tr class="mailfield-rules-validateNgRegex">
			<th>正規表現（NGワード）</th>
			<td>
				/ <?php echo $this->BcForm->input('ngRegex', ['size' => 50]) ?> /us <?php echo $this->BcForm->error('noRegex') ?><br>
		...
		<tr class="mailfield-rules-convertKana">
			...
			<td>
				...
				<small><a href="http://php.net/manual/ja/function.mb-convert-kana.php" target=_blank>mb_convert_kana関数</a>の変換オプションを指定してください。</small>
			</td>
		</tr>
		...
		<tr class="mailfield-rules-convertRegex">
			<th>正規表現</th>
			<td>
				from / <?php echo $this->BcForm->input('convertRegexPattern', ['size' => 50]) ?> /us <?php echo $this->BcForm->error('convertRegexPattern') ?><br>
				to <?php echo $this->BcForm->input('convertRegexReplacement', ['size' => 50]) ?> <?php echo $this->BcForm->error('convertRegexReplacement') ?><br>
			</td>
		</tr>
		<tr class="mailfield-rules-convertDate">
			<th>日付</th>
			<td>
				<?php echo $this->BcForm->input('convertDateOption', ['size' => 10]) ?><?php echo $this->BcForm->error('convertDateOption') ?><br>
				<small><a href="http://php.net/manual/ja/function.date.php" target=_blank>date関数</a>関数のフォーマットを指定してください。</small>
			</td>
		</tr>
		<tr class="mailfield-rules-sendLimitTimes">
			<th>連続送信制限</th>
			<td>
				<?php echo $this->BcForm->input('sendLimitTime', ['size' => 5]) ?><?php echo $this->BcForm->error('sendLimitTime') ?> 分に
				<?php echo $this->BcForm->input('sendLimitTimes', ['size' => 5]) ?><?php echo $this->BcForm->error('sendLimitTimes') ?> 回まで送信を許可する
			</td>
		</tr>
		<tr class="mailfield-rules-sendLimitIP">
			<th>IP制限</th>
			<td>
				<?php echo $this->BcForm->input('sendLimitIP', ['type' => 'textarea']) ?><?php echo $this->BcForm->error('sendLimitIP') ?><br>
				<small>一行にひとつずつIPを入力してください。</small>
			</td>
		</tr>
	</table>

	<table cellpadding="0" cellspacing="0" class="form-table  craft-form-rule-table-error-message">
			<th>エラーメッセージ</th>
			<td>
